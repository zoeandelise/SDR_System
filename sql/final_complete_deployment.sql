...
ADD COLUMN IF NOT EXISTS `vitamin_b1` decimal(8,4) DEFAULT NULL COMMENT '维生素B1(mg/100g)' AFTER `potassium`,
ADD COLUMN IF NOT EXISTS `omega_3` decimal(8,4) DEFAULT NULL COMMENT 'Omega-3脂肪酸(g/100g)' AFTER `vitamin_b1`,
ADD COLUMN IF NOT EXISTS `glycemic_index` int(3) DEFAULT NULL COMMENT '血糖指数GI' AFTER `omega_3`;

-- 1.3 为食物信息表添加营养字段
ALTER TABLE `diet_food_info` 
ADD COLUMN IF NOT EXISTS `calories_per_100g` decimal(8,2) DEFAULT NULL COMMENT '热量(kcal/100g)' AFTER `standard_weight`,
ADD COLUMN IF NOT EXISTS `protein_per_100g` decimal(8,2) DEFAULT NULL COMMENT '蛋白质(g/100g)' AFTER `calories_per_100g`,
ADD COLUMN IF NOT EXISTS `fat_per_100g` decimal(8,2) DEFAULT NULL COMMENT '脂肪(g/100g)' AFTER `protein_per_100g`,
ADD COLUMN IF NOT EXISTS `carbohydrate_per_100g` decimal(8,2) DEFAULT NULL COMMENT '碳水化合物(g/100g)' AFTER `fat_per_100g`,
ADD COLUMN IF NOT EXISTS `fiber_per_100g` decimal(8,2) DEFAULT NULL COMMENT '纤维(g/100g)' AFTER `carbohydrate_per_100g`;
